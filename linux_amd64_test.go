...
func TestApplicationServiceForLinuxAmd64(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.repo, func(t *testing.T) {
			...
			asset, err := main.NewAssetRepository(tt.repo, io.Discard)
			require.NoError(err)
			app := main.NewApplicationService(asset, main.NewExecBinaryRepository(dir))

			ctx := context.Background()

			result, err := app.Find(ctx, tt.tag, main.DefaultPatterns)
			require.NoError(err)
			require.Equal(tt.result, result)

			err = app.Install(ctx, result)
			require.NoError(err)
